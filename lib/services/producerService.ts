...
import { normalizeMarkdownDeliverable, normalizeRepoPath } from "@/lib/utils";
import { inferTaskAssignment } from "@/lib/services/taskAssignmentService";
import { isCrewAIBacklogPlannerEnabled, planBacklogWithCrewAI } from "@/lib/services/backlogPlanningService";
...
import type { BacklogPlanningWave } from "@/lib/types/planning";
...
export function pickAgent(
  ...
  preferredSpecialization?: ProgrammerSpecialization
): Agent | null {
  const isAvailableAgent = (agent: Agent) =>
    ["active", "actif", "recruté", "recrute", "onboarding"].includes(
      (agent.status ?? "").toLowerCase()
    );

  const deptAgents = agents.filter(
    (a) => a.department === department && isAvailableAgent(a)
  );
  ...
  // For programming department: match by specialization
  if (department === "programming") {
    // Si une spécialisation préférée est fournie, chercher cet agent en priorité
    if (preferredSpecialization) {
      const specMatch = pool.find((a) => (a as Agent & { specialization?: string }).specialization === preferredSpecialization);
      if (specMatch) return specMatch;
    }
    // Sinon, essayer de déduire la spécialisation depuis le titre/description
    else if (taskTitle) {
      const contentLower = (taskDescription ? `${taskTitle} ${taskDescription}` : taskTitle).toLowerCase();
      ...
      // Trouver la première spécialisation qui matche avec les mots-clés du contenu
      for (const [specId, keywords] of Object.entries(SPEC_KEYWORDS)) {
        if (keywords.some((kw) => contentLower.includes(kw))) {
          // Chercher un agent avec CETTE spécialisation précise
    ...
    // Si aucun agent ne correspond à la spécialisation, retourner le premier agent du pool (par position)
    // Cela permet d'assigner une tâche même si personne n'a la spécialisation exacte
  }
...
export async function generateConceptPipeline(project: Project): Promise<PipelineTask[]> {
  ...
  for (const doc of CONCEPT_DOCS) {
    // Utiliser inferTaskAssignment pour déterminer automatiquement le département et la spécialisation
    const assignment = inferTaskAssignment(doc.title, doc.description, doc.agentDepartment);
...
interface DevTaskDef {
  ...
  specialization?: "gameplay" | "engine" | "backend" | "ui-tech" | "devops" | null;
...
function buildNextWavePrompt(params: {
  ...
  const agentsList = agents
    .filter((agent) => {
      ...
      return ["active", "actif", "recruté", "recrute", "onboarding"].includes(status);
    })
...
- Pour les tâches de programmation, spécifie la spécialisation : "gameplay", "engine", "backend", "ui-tech", "devops" ou null si non applicable
...
function convertCrewAIWavesToDevWaveDefs(waves: BacklogPlanningWave[]): DevWaveDef[] {
  return waves.map((wave) => ({
    number: wave.number,
    tasks: wave.tasks.map((task) => ({
      title: task.title,
      description: task.description,
      backlog_ref: task.backlog_ref,
      assigned_agent_slug: null,
      agent_department: task.agent_department,
      specialization: (task.specialization as DevTaskDef["specialization"]) ?? null,
      deliverable_type: task.deliverable_type,
      deliverable_path: task.deliverable_path,
      context_files: task.context_files,
      depends_on_refs: task.depends_on_refs,
    })),
  }));
}
...
async function createDevTasksFromWaveDefs(
  ...
  const allDefs = waves.flatMap((wave) => wave.tasks.map((task) => ({ ...task, waveNumber: wave.number })));
  const createdTasks: PipelineTask[] = [];
  const refToTaskId = new Map<string, string>();

  for (let index = 0; index < allDefs.length; index++) {
    const definition = allDefs[index];
    const dependsOn: string[] = (definition.depends_on_refs ?? [])
      .map((reference: string) => refToTaskId.get(reference))
      .filter(Boolean) as string[];
    ...
    const assignment = inferTaskAssignment(
      ...
      definition.agent_department ?? undefined
    );
    const assignedAgent = pickAgent(
      assignment.agentDepartment,
      agents,
    ...
    const task = await createTask({
      ...
      backlogRef: definition.backlog_ref ?? null,
      projectPhase: "in-dev",
      waveNumber: definition.waveNumber,
      sortOrder: index,
      status,
      requiresReview: false,
      assignedAgentSlug: assignedAgent?.slug ?? definition.assigned_agent_slug ?? null,
      agentDepartment: assignment.agentDepartment,
      llmModel: LLM_MODELS.tasks,
      llmPromptTemplate: null,
      llmContextFiles: definition.context_files ?? [],
      deliverableType: normalizeDeliverableType(definition.deliverable_type, definition.deliverable_path),
      deliverablePath: definition.deliverable_path ? normalizeRepoPath(definition.deliverable_path) : null,
      deliverableContent: null,
      dependsOn,
    });

    createdTasks.push(task);
    if (definition.backlog_ref) refToTaskId.set(definition.backlog_ref, task.id);
  }
...
export async function generateDevWaves(projectId: string, project: Project): Promise<PipelineTask[]> {
  ...
  if (isCrewAIBacklogPlannerEnabled()) {
    ...
    if (crewResult) {
      waves = convertCrewAIWavesToDevWaveDefs(crewResult.waves);
    } else {
  ...
  return createDevTasksFromWaveDefs(projectId, waves, agents);
}
