...
export function CollaborateurPageClient({
  initialAgents,
  initialActiveProjectsCount,
}: {
  ...
  const deleteAgent = async (slug: string, name: string) => {
    ...
    try {
      ...
      if (res.ok) {
        setAgents((prev) => prev.filter((agent) => agent.slug !== slug));
      }
    } catch {
      // silent fail
    } finally {
        ...
        <div className="group relative bg-card border border-white/12 rounded-2xl p-5 md:p-7 hover:bg-white/8 transition-colors overflow-hidden">
          ...
          <div className="space-y-3 mt-4 pt-4 border-t border-white/8">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="font-bold text-white text-base">{initialActiveProjectsCount}</span>
            </div>
          </div>
        </div>

        {agents.map((agent) => {
          const dept = departments.find((department) => department.id === agent.department);
