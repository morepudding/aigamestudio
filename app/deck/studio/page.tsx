...
export default function StudioDeckPage() {
  ...
  const fetchCard = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/deck/propose", {
        ...
        body: JSON.stringify({ agentSlug: "studio" }),
      });
  ...
  const decide = useCallback(
    async (accepted: boolean) => {
      ...
      setDeciding(true);
      setDecision(accepted ? "accepted" : "refused");
  ...
  const acceptedCount = history.filter((c) => c.accepted).length;
      ...
      {acceptedCount > 0 && (
        <div className="px-4 py-1">
          <span className="text-xs text-emerald-400/70 bg-emerald-400/10 px-2 py-0.5 rounded-full">
            +{acceptedCount} carte{acceptedCount > 1 ? "s" : ""} ajoutée{acceptedCount > 1 ? "s" : ""}
          </span>
        </div>
      )}
          ...
          {!loading && card && (
            ...
            >
              ...
              <div className={`flex items-center gap-1.5 mb-4 text-xs font-medium ${meta?.color}`}>
                ...
                <span className="ml-auto text-white/20 text-[10px]">studio</span>
              </div>
              ...
              {card.themes.length > 0 && (
                <div className="flex flex-wrap gap-1 mt-4">
                  {card.themes.map((t) => (
                    <span key={t} className="text-[10px] text-white/30 bg-white/5 px-2 py-0.5 rounded-full">
                      {t}
                    </span>
                  ))}
                </div>
        ...
        <footer className="px-4 pb-6">
          ...
          <div className="flex flex-col gap-2">
            {history.slice(0, 5).map((h) => {
              ...
              return (
                <div key={h.id} className="flex items-start gap-2 text-xs text-white/40 bg-white/3 rounded-lg px-3 py-2">
