...
import { getAllConversations } from "@/lib/services/chatService";
...
export default function ChatPage() {
  ...
  const getConvForAgent = (slug: string) =>
    conversations.find((c) => c.agentSlug === slug);
  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-none">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3 md:gap-5">
        {agents.map((agent) => {
          const gradient =
            departmentGradients[agent.department] ??
            "from-gray-500 to-gray-600";
          const initials = getInitials(agent.name);
          const conv = getConvForAgent(agent.slug);
          const lastMsg = conv?.messages[conv.messages.length - 1];
          const isAwaiting = conv?.awaitingUserReply ?? false;
          const discoveryCount = conv?.messages.filter((m) => m.messageType === "discovery").length ?? 0;
              ...
              <div className="p-4 md:p-5">
                ...
                <div className="flex items-start gap-4 mb-4">
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-12 h-12 rounded-full bg-gradient-to-br ${gradient} flex items-center justify-center text-white text-sm font-bold shadow-lg overflow-hidden`}
                    >
                      {agent.icon_url ? (
                        <Image
                          src={agent.icon_url}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground/50 truncate mb-1.5">
                      {agent.role}
                    </p>
                    <span
                      className={`inline-flex items-center px-2 py-0.5 rounded-full bg-gradient-to-r ${gradient} bg-opacity-10 text-[10px] font-medium text-white/80`}
                    >
                  ...
                  <div className="flex-1 min-w-0">
                    {lastMsg ? (
                      <p className="text-[11px] text-muted-foreground/50 truncate">
                        {lastMsg.sender === "user" && (
                          <span className="text-muted-foreground/30">Vous : </span>
                        )}
